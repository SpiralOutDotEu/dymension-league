...
contract GameLeagueTest is Test {
    ...
    function testCreateTeam(string memory teamName, bool approveAll) public {
        ...
        assertEq(
            cosmoShips.ownerOf(1),
            address(gameLeague),
            "NFT 1 should be staked"
        );
        assertEq(
            cosmoShips.ownerOf(2),
            address(gameLeague),
            "NFT 2 should be staked"
        );
        assertEq(
            cosmoShips.ownerOf(3),
            address(gameLeague),
            "NFT 3 should be staked"
        );
        // Check that team is created with correct values
        (
            string memory retrievedName,
            uint256[] memory retrievedNftIds,
            address retrievedOwner
        ) = gameLeague.getTeam(0);
        assertEq(retrievedName, teamName, "Not correct teamName");
        assertEq(retrievedOwner, user, "Owner of the team does not match");
        assertEq(
            retrievedNftIds.length,
            3,
            "Number of NFTs in the team should be 3"
        );
    ...
    function tokenMint(
        ...
        require(
            ...
            "Token IDs and attributes length mismatch"
        );
        for (uint256 i = 0; i < tokenIds.length; i++) {
            vm.prank(recipient);
            cosmoShips.mint{value: mintPrice}(attributes[i], proof);
            assertEq(
                cosmoShips.attributes(tokenIds[i]),
                attributes[i],
    ...
    function testEnrollToLeague() public {
        ...
        assertTrue(
            gameLeague.isTeamEnrolled(teamId, gameLeague.currentLeagueId())
        );
    }
    ...
    function testBetPlacing() public {
        ...
        (
            uint256[] memory betTeamIdsAlice,
            uint256[] memory betAmountsAlice
        ) = gameLeague.getUserBets(leagueId, alice);
        assertEq(betTeamIdsAlice[0], teamId, "Alice's Team ID should match");
        assertEq(
            betAmountsAlice[0],
            betAmountAlice,
            "Alice's bet amount should match"
        );
        vm.stopPrank();
        ...
        gameLeague.placeBet(leagueId, teamId, betAmountBob / 2);
        (
            uint256[] memory betTeamIdsBob,
            uint256[] memory betAmountsBob
        ) = gameLeague.getUserBets(leagueId, bob);
        assertEq(betTeamIdsBob[0], teamId, "Bob's Team ID should match");
        assertEq(
            betAmountsBob[0],
            betAmountBob,
            "Bob's bet amount should match"
        );
        vm.stopPrank();
    ...
    function testMatchSetupAndOutcome() public {
        ...
        (
            ,
            GameLeague.LeagueState state,
            ,
            uint256[] memory enrolledTeams,

        ) = gameLeague.getLeague(leagueId);
